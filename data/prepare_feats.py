...
def make_feature(wav_path_list, noise_wav_list, feat_dir, thread_num, argument=False, repeat_num=1):
    ...
    with kaldi_io.open_or_fd(mag_ark_scp_output,'wb') as f_mag, kaldi_io.open_or_fd(ang_ark_scp_output,'wb') as f_ang:
        for num in range(repeat_num):
            for tmp in wav_path_list:
                uttid, wav_path = tmp            
                clean = load_audio(wav_path)
                if len(clean) == 0:
                    print("%s is EMPTY!!"%(wav_path)) 
                    return None
                y = None
                while y is None:
                    if argument:
...
def main():
    ...
    for i in range(threads_num):
        ...
        #make_feature(wav_path_tmp_list, noise_wav_list, clean_feat_dir, i, False)
